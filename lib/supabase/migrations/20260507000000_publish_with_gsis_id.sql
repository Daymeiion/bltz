-- =============================================================================
-- 20260507000000_publish_with_gsis_id.sql
-- Extend the publish RPC to persist `players.gsis_id`.
--
-- Background: the nflverse scraper now emits the matched player's GSIS ID
-- through the pipeline. This migration replaces `publish_onboarding_run` with
-- a version that reads the new `gsis_id` field off the player payload jsonb
-- and writes it during both the insert and update branches. All other
-- behavior is identical to the previous revision (atomic publish, idempotent
-- award/media inserts, claim-token settlement, runs-row finalization).
-- =============================================================================
...
begin
  ...
  if v_player_id is null then
    insert into players (
      ...
      profile_image, youtube_urls, gsis_id, visibility, is_public, user_id,
      confirmed_fields
    )
    values (
    ...
    update players
       set full_name = p_player->>'full_name', name = p_player->>'name',
           ...
           -- gsis_id only updated when the new payload supplies one; never
           -- nullified by an empty re-publish (preserves the link if a
           -- subsequent edit drops the field for any reason).
