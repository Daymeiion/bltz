-- =============================================================================
-- 20260505000002_atomic_onboarding_publish.sql
-- Atomic + idempotent onboarding publish.
--
-- The HTTP route calls `publish_onboarding_run` so the player/profile/token/run
-- promotion and media/award writes commit together. Retried publishes use unique
-- indexes and `on conflict` handling to avoid duplicate review media or awards.
-- =============================================================================

create unique index if not exists player_awards_player_identity_idx
  on player_awards (
    player_id,
    lower(name),
    coalesce(year, -1),
...
create unique index if not exists media_player_kind_url_idx
  on media (player_id, kind, url);

create unique index if not exists onboarding_pipeline_runs_claim_user_idx
  on onboarding_pipeline_runs (claim_token, user_id)
  where claim_token is not null;

create unique index if not exists onboarding_pipeline_runs_active_claim_token_idx
  on onboarding_pipeline_runs (claim_token)
  where claim_token is not null
...
begin
  ...
  if v_player_id is null then
    ...
    values (
      ...
      nullif(p_player->>'height_in', '')::smallint,
      nullif(p_player->>'weight_lbs', '')::smallint,
      nullif(p_player->>'games_played', '')::integer,
      nullif(p_player->>'position', ''),
      nullif(p_player->>'level', ''),
      nullif(p_player->>'school', ''),
      v_school_id,
      nullif(p_player->>'hometown', ''),
      nullif(p_player->>'headshot_url', ''),
      nullif(p_player->>'image_url', ''),
      nullif(p_player->>'profile_image', ''),
      coalesce(p_player->'youtube_urls', '[]'::jsonb),
      true,
      true,
      p_user_id,
      coalesce(p_player->'confirmed_fields', '{}'::jsonb)
    )
    returning id into v_player_id;
  else
    update players
       set full_name = p_player->>'full_name',
           name = p_player->>'name',
           slug = v_slug,
           bio = p_player->>'bio',
  ...
  loop
    ...
    values (
      v_player_id,
      v_award->>'name',
      nullif(v_award->>'year', '')::integer,
      nullif(v_award->>'organization', ''),
      nullif(v_award->>'source_url', ''),
      true,
      false,
      'sports',
      'regional'
  ...
  loop
    ...
    values (
      ...
      v_photo->>'url',
      'photo',
      nullif(v_photo->>'credits', ''),
      nullif(v_photo->>'width', '')::integer,
      nullif(v_photo->>'height', '')::integer,
      coalesce((v_photo->>'display_order')::integer, 1),
      'founder_archive'
    )
    on conflict do nothing;
  end loop;
