...
- Final runtime build: `LpYwrECrdgU6o6W0fBHfY`, built from `b7ef789`. Later commits are release documentation/generator/proof scripts only.
...
- **14 exact deployment-packet assertions** pass in a third dedicated local database: 27 released migrations plus 20 inert synthetic ledger entries model a 47-row baseline. Only the ordered-version fingerprint is substituted; full migration DDL, count/latest guards, audit functions, permissions and ledger append remain exact. A deliberate pre-COMMIT error rolls back tables/functions/ledger; successful application appends 47→48 with old history unchanged; retry fails without mutation. No fixture deletion/reset or hosted access. Evidence: `C:/tmp/bltz-preview-packet-20260831/packet-proof.json`.
...
3. Generate an exact review packet locally with `node scripts/prepare-preview-deployment.mjs <explicit-outside-repository.sql>`. The generator connects to no database. It embeds the committed migration, sets a 5-second lock timeout and 45-second statement timeout, locks the ledger, checks the exact baseline, applies only the new DDL and appends one standard new-version ledger row in the same transaction. A final permission/ledger guard precedes COMMIT. Existing 47 rows are untouched. Do not auto-retry a packet if any version/object now exists.
4. Coordinator reviews packet/migration hashes and obtains the release gate before SQL execution or publication. The production packet is **not applied to production by this task**; its complete body/wrapper were verified locally using only a synthetic baseline fingerprint substitution. The hosted fingerprint still requires fresh read-only confirmation. LF-normalized migration SHA256: `518e27b94674ff31b0f55af12f408b71e8c6339f51bdd7226a88cce72018b54a`. Bounded packet SHA256: `83ba877e033b6ca9249e549b65da60470319e61403f84ef70613d70592ec9155`.
