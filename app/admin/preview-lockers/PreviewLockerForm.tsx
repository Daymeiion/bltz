...
import { SportradarPanel } from "./[id]/edit/SportradarPanel";
import PreviewViewerAccess from "./PreviewViewerAccess";
import CfbCsvImport from "./CfbCsvImport";
...
type Scalar = Exclude<keyof PreviewContent, "schools" | "pro_teams" | "awards" | "career_stats" | "cfb_stats" | "videos" | "photos">;
...
export default function PreviewLockerForm({
  ...
  const [busy, setBusy] = useState<"discovery" | "upload" | "save" | "complete" | "stats" | null>(null);
  const [showStats, setShowStats] = useState(false);
  const [statsNeedReload, setStatsNeedReload] = useState(false);
  ...
  return <div className="space-y-6">
    ...
    <fieldset disabled={!!busy || statsNeedReload} className="space-y-6 disabled:opacity-70">
      ...
      <div className="grid gap-4 sm:grid-cols-2">{field("headshot_url", "Headshot HTTPS URL", 2048)}{field("hero_video_url", "Desktop landscape hero URL (legacy fallback)", 2048)}</div>
      ...
      {(["schools", "pro_teams"] as const).map(kind => <BuilderSection key={kind} title={kind === "schools" ? "School history" : "Pro team history"} count={draft[kind].length} description="Expand to review and edit career affiliations">{draft[kind].map((team, index) => <div key={index} className="grid gap-2 sm:grid-cols-4"><Input aria-label={`${kind} ${index + 1} label`} value={team.label} maxLength={80} onChange={e => change({ [kind]: draft[kind].map((v, i) => i === index ? { ...v, label: e.target.value } : v) })} /><Input aria-label={`${kind} ${index + 1} color`} type="color" value={team.color} onChange={e => change({ [kind]: draft[kind].map((v, i) => i === index ? { ...v, color: e.target.value } : v) })} /><Input aria-label={`${kind} ${index + 1} logo HTTPS URL`} value={team.logo ?? ""} maxLength={2048} onChange={e => change({ [kind]: draft[kind].map((v, i) => i === index ? { ...v, logo: e.target.value || null } : v) })} /><Button type="button" variant="outline" onClick={() => change({ [kind]: draft[kind].filter((_, i) => i !== index) })}>Remove from draft</Button></div>)}<Button type="button" variant="outline" disabled={draft[kind].length >= 12} onClick={() => change({ [kind]: [...draft[kind], { label: "", color: "#152238", logo: null }] })}>Add {kind === "schools" ? "school" : "pro team"}</Button></BuilderSection>)}
      <BuilderSection title="Awards" count={draft.awards.length} description="Expand to review and edit achievements">{draft.awards.map((award, index) => <div key={index} className="grid gap-2 sm:grid-cols-3"><Input aria-label={`Award ${index + 1} year`} maxLength={20} value={award.year} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, year: e.target.value } : v) })} /><Input aria-label={`Award ${index + 1} label`} maxLength={200} value={award.label} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, label: e.target.value } : v) })} /><Input placeholder="What does this award recognize?" aria-label={`Award ${index + 1} description`} maxLength={160} value={award.description ?? ""} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, description: e.target.value } : v) })} /><Input type="url" placeholder="Related article URL (optional)" aria-label={`Award ${index + 1} article URL`} value={award.sourceUrl ?? ""} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, sourceUrl: e.target.value || null } : v) })} /><Button type="button" variant="outline" onClick={() => change({ awards: draft.awards.filter((_, i) => i !== index) })}>Remove from draft</Button></div>)}<Button type="button" variant="outline" disabled={draft.awards.length >= 40} onClick={() => change({ awards: [...draft.awards, { year: "", label: "" }] })}>Add award</Button></BuilderSection>
      <CfbCsvImport athleteName={draft.full_name} value={draft.cfb_stats} onChange={cfb_stats => change({ cfb_stats })} />
      <section className="space-y-3" aria-label="Stats-only update">
      ...
      <MediaSection title="Videos" count={draft.videos.length} limit={24}>{saved ? <label className="grid gap-2 text-sm font-medium">Upload multiple private videos<Input aria-label="Upload multiple private videos" type="file" multiple accept="video/mp4,video/webm,video/quicktime" onChange={event => { void uploadFiles("video", event.target.files); event.target.value = ""; }} /><span className="font-normal">MP4, WebM, or MOV. Up to 250 MB each. Uploads stay private and must be saved into this draft.</span></label> : <p className="text-sm">Save the private preview once before uploading video files.</p>}{draft.videos.map((video, index) => <MediaItem key={video.id} media={video} kind="video" index={index}><label className="grid gap-2 text-sm">Hero placement<select className="rounded-md border bg-background p-2" value={video.heroDevice ?? ""} onChange={event => {
        const device = event.target.value as "mobile" | "desktop" | "";
        change({ videos: draft.videos.map((item, i) => {
          const { heroDevice: previous, ...rest } = item;
          return i === index ? { ...rest, ...(device ? { heroDevice: device } : {}) } : previous === device ? rest : item;
        }) });
      }}><option value="">Film Room only</option><option value="mobile">Mobile hero · portrait (9:16)</option><option value="desktop">Desktop / tablet hero · landscape (16:9)</option></select><span>Use a direct MP4/WebM file or a private upload. Select the matching orientation; devices without a selection show photos.</span></label><Input aria-label={`Video ${index + 1} title`} placeholder="Title" maxLength={160} value={video.title} onChange={e => change({ videos: draft.videos.map((v, i) => i === index ? { ...v, title: e.target.value } : v) })} />{"url" in video ? <><Input aria-label={`Video ${index + 1} url`} placeholder="url HTTPS URL" maxLength={2048} value={video.url} onChange={e => change({ videos: draft.videos.map((v, i) => i === index ? { ...v, url: e.target.value } : v) })} /><Input aria-label={`Video ${index + 1} thumb`} placeholder="thumb HTTPS URL" maxLength={2048} value={video.thumb ?? ""} onChange={e => change({ videos: draft.videos.map((v, i) => i === index && "url" in v ? { ...v, thumb: e.target.value || null } : v) })} /></> : <p className="text-sm">Private uploaded file · {video.mimeType}</p>}<Button type="button" variant="outline" onClick={() => change({ videos: draft.videos.filter((_, i) => i !== index) })}>Remove video from draft</Button></MediaItem>)}<Button type="button" variant="outline" disabled={draft.videos.length >= 24} onClick={() => change({ videos: [...draft.videos, { id: crypto.randomUUID(), title: "", url: "", thumb: null }] })}>Add video</Button></MediaSection>
      <MediaSection title="Photos" count={draft.photos.length} limit={40}>{saved ? <label className="grid gap-2 text-sm font-medium">Upload multiple private photos<Input aria-label="Upload multiple private photos" type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={event => { void uploadFiles("photo", event.target.files); event.target.value = ""; }} /><span className="font-normal">JPG, PNG, or WebP. Up to 10 MB each. Uploads stay private and must be saved into this draft.</span></label> : <p className="text-sm">Save the private preview once before uploading photo files.</p>}{draft.photos.map((photo, index) => <MediaItem key={photo.id} media={photo} kind="photo" index={index}><Input aria-label={`Photo ${index + 1} title`} placeholder="title" maxLength={160} value={photo.title} onChange={e => change({ photos: draft.photos.map((v, i) => i === index ? { ...v, title: e.target.value } : v) })} />{"url" in photo ? <Input aria-label={`Photo ${index + 1} url`} placeholder="url" maxLength={2048} value={photo.url} onChange={e => change({ photos: draft.photos.map((v, i) => i === index ? { ...v, url: e.target.value } : v) })} /> : <p className="text-sm">Private uploaded file · {photo.mimeType}</p>}{(["credits", "sourceUrl", "season"] as const).map(key => <Input key={key} aria-label={`Photo ${index + 1} ${key}`} placeholder={key} maxLength={key === "credits" ? 300 : key === "season" ? 20 : 2048} value={photo[key] ?? ""} onChange={e => change({ photos: draft.photos.map((v, i) => i === index ? { ...v, [key]: e.target.value || null } : v) })} />)}<select aria-label={`Photo ${index + 1} level`} className="h-10 rounded-md border bg-background px-3" value={photo.level} onChange={e => change({ photos: draft.photos.map((v, i) => i === index ? { ...v, level: e.target.value as typeof photo.level } : v) })}>{["hs", "cfb", "pro", "off-field"].map(level => <option key={level}>{level}</option>)}</select><Button type="button" variant="outline" onClick={() => change({ photos: draft.photos.filter((_, i) => i !== index) })}>Remove photo from draft</Button></MediaItem>)}<Button type="button" variant="outline" disabled={draft.photos.length >= 40} onClick={() => change({ photos: [...draft.photos, { id: crypto.randomUUID(), title: "", url: "", credits: null, sourceUrl: null, season: null, level: "off-field" }] })}>Add photo</Button></MediaSection>
