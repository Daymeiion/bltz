...
export default function PreviewLockerForm({
  ...
  return <div className="space-y-6">
    ...
    <fieldset disabled={!!busy || statsNeedReload} className="space-y-6 disabled:opacity-70">
      ...
      <section className="space-y-3"><h2 className="font-semibold">Awards</h2>{draft.awards.map((award, index) => <div key={index} className="grid gap-2 sm:grid-cols-3"><Input aria-label={`Award ${index + 1} year`} maxLength={20} value={award.year} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, year: e.target.value } : v) })} /><Input aria-label={`Award ${index + 1} label`} maxLength={200} value={award.label} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, label: e.target.value } : v) })} /><Input placeholder="What does this award recognize?" aria-label={`Award ${index + 1} description`} maxLength={160} value={award.description ?? ""} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, description: e.target.value } : v) })} /><Input type="url" placeholder="Related article URL (optional)" aria-label={`Award ${index + 1} article URL`} value={award.sourceUrl ?? ""} onChange={e => change({ awards: draft.awards.map((v, i) => i === index ? { ...v, sourceUrl: e.target.value || null } : v) })} /><Button type="button" variant="outline" onClick={() => change({ awards: draft.awards.filter((_, i) => i !== index) })}>Remove from draft</Button></div>)}<Button type="button" variant="outline" disabled={draft.awards.length >= 40} onClick={() => change({ awards: [...draft.awards, { year: "", label: "" }] })}>Add award</Button></section>
