...
export function PreviewVideoSource({ video }: { video: { id: string; title: string; url: string } }) {
  const embed = youtubeEmbed(video.url);
  return <section id={video.id} className="space-y-3 rounded-lg border border-white/20 p-4"><h2>{video.title}</h2>{embed ? <iframe className="aspect-video w-full" src={embed} title={video.title} loading="lazy" referrerPolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-presentation" allow="fullscreen" allowFullScreen /> : <p>This source is not loaded inside BLTZ. Open it on its provider to review it.</p>}<a className="underline" href={video.url} target="_blank" rel="noopener noreferrer">Open source · rights unverified</a></section>;
}
