...
function ActiveGtmContactDrawer({
  ...
  function submitEdit(formData: FormData) {
    const contactType = contact.playerMaster ? "athlete" : editContactType;
    const input = {
      contactId: contact.id,
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 right-0 z-[70] w-full overflow-y-auto border-l border-neutral-200 bg-[#f7f6f3] p-5 text-neutral-950 shadow-2xl focus:outline-none sm:max-w-[42rem] sm:p-7 dark:border-neutral-800 dark:bg-neutral-950 dark:text-white">
          ...
          <div className="mt-3 flex flex-wrap gap-2 border-b border-neutral-200 pb-5 dark:border-neutral-800">
            {contact.contactType === "athlete" && !contact.playerMaster && <button type="button" onClick={() => openMode("player")} className="min-h-11 rounded-xl px-3 text-sm font-semibold underline-offset-4 hover:underline"><IconLink className="mr-1 inline h-4 w-4" />Match Player</button>}
          ...
          {contact.playerMaster && <section className="mt-4 rounded-2xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900" aria-label="Player Master identity"><div className="flex items-start gap-3"><IconUserCheck className="mt-0.5 h-5 w-5" /><div><h3 className="font-semibold">Player Master identity</h3><p className="mt-2 text-sm font-semibold">{contact.playerMaster.displayName}</p><p className="mt-1 text-xs text-neutral-500">{[contact.playerMaster.position, contact.playerMaster.team, contact.playerMaster.collegeName].filter(Boolean).join(" · ") || "Canonical athlete record"}</p><p className="mt-2 font-mono text-[11px] text-neutral-500">{contact.playerMaster.gsisId}</p></div></div></section>}
          ...
          {mode === "stage" && <form action={submitStage} className={formShell}><h3 className="font-semibold">Change Pipeline Stage</h3><p className="text-sm text-neutral-500">{GTM_PIPELINE_DESCRIPTION}</p><label className="block text-sm font-medium">Stage<select name="pipelineStage" defaultValue={contact.pipelineStage ?? "identified"} className={inputClass}>{GTM_PIPELINE_STAGES.map((stage) => <option key={stage} value={stage}>{label(stage)}</option>)}</select></label><FormActions pending={pending} submitLabel="Save stage" onCancel={() => setMode(null)} /></form>}
          ...
          {contact.contactType === "enterprise" && <section className="mt-8"><h3 className="text-lg font-semibold">Enterprise priority factors</h3><p className="mt-2 text-xs text-neutral-500">{contact.priorityFactorsReviewed ? "Reviewed factors" : "Unreviewed factors — imported values are suggestions"}. Relationship strength is assessed manually; this is not a Locker engagement score.</p><div className="mt-3 grid grid-cols-2 gap-3">{[["Relationship", contact.relationshipStrength], ["BLTZ relevance", contact.bltzRelevance], ["Buying authority", contact.buyingAuthority], ["Network leverage", contact.networkLeverage], ["Timing", contact.timingScore]].map(([term, value]) => <div key={String(term)} className="rounded-xl border border-neutral-200 bg-white p-3 dark:border-neutral-800 dark:bg-neutral-900"><p className="text-xs text-neutral-500">{term}</p><p className="mt-1 font-mono text-lg font-semibold">{value == null ? "–" : String(value)}</p></div>)}</div></section>}
