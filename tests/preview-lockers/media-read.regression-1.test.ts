...
beforeEach(() => {
  vi.resetAllMocks(); mocks.rpc.mockResolvedValue({data:900,error:null}); mocks.auth.mockResolvedValue({ data: { user: { id: "viewer" } }, error: null });
...
it("caps signing at the remaining grant lifetime and rejects expired grants", async () => {
 mocks.rpc.mockResolvedValue({data:12,error:null}); mocks.signed.mockResolvedValue({data:[{signedUrl:"https://storage.example/signed"}],error:null});
 await readPrivatePreview("fixture-player"); expect(mocks.signed).toHaveBeenCalledWith([path],12);
 mocks.signed.mockClear(); mocks.rpc.mockResolvedValue({data:0,error:null});
 await expect(readPrivatePreview("fixture-player")).rejects.toMatchObject({code:"preview_expired"}); expect(mocks.signed).not.toHaveBeenCalled();
});
